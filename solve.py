...
def solve(destination,c1,c2,c3,PUNISHMENT_VD,PUNISHMENT_VN,PUNISHMENT_ND=5/31746):
    ...
    M=100000
    if destination!=3:
        #vehicle_ub=max(max_vehicles_calculation(demands=demands,destination=i,vehicles=vehicles) for i in range(4))
    ...
    # for l in range(vehicle_ub):
    #     if l <vehicle_ub/2:
    #         IA[l].start=1
    #     else:
    #         IA[l].start=0
    LA = model.addVars(vehicle_ub, len(sku_des), vtype=GRB.INTEGER,lb=0, ub=max_sku_qty, name="Loading_amount")
    ...
    #车辆未激活时LA为0
    model.addConstrs((LA[v, i] <= IA[v] * max_sku_qty for v in range(vehicle_ub) for i in range(len(sku_des))), f"active_loading_{v}_{i}")
    ...
    # 3.-ND.sum()最大化
    if destination==3: #订单发货量最大化:destinatino==3的时候被ND罚数满足, destination!=3时被订单满足约束满足
        #obj1=quicksum(LA[v,i]*IA[v] for v in range(vehicle_ub) for i in range(len(sku_des)))
        obj1=0
    else:
        obj1=0
    obj2=-IA.sum()
